...
///   <item>Non-sliced inputs (offset = 0).</item>
...
{
    /// <summary>
    /// True iff the column is non-nullable, non-sliced, and all values are
    /// byte-identical at the value buffer level.
    /// </summary>
        ...
        SegmentBuilder sb, IArrowArray array, ushort constantEncodingIdx, int? statsTicket = null)
    {
        ...
        if (!IsApplicable(array))
            throw new InvalidOperationException(
                $"vortex.constant writer requires a non-nullable, non-sliced column with all values equal; {array.GetType().Name} doesn't qualify.");
    ...
    {
        int n = data.Length;
        var span = data.Buffers[1].Span.Slice(0, n * elemSize);
    ...
    private static byte[] SerializeFirstValue(IArrowArray array)
    {
        ...
        return array switch
        {
            ...
            Int8Array => ScalarValueSerializer.FromSignedInt((sbyte)data.Buffers[1].Span[0]),
            Int16Array => ScalarValueSerializer.FromSignedInt(MemoryMarshal.Read<short>(data.Buffers[1].Span)),
            Int32Array => ScalarValueSerializer.FromSignedInt(MemoryMarshal.Read<int>(data.Buffers[1].Span)),
            Int64Array => ScalarValueSerializer.FromSignedInt(MemoryMarshal.Read<long>(data.Buffers[1].Span)),
            UInt8Array => ScalarValueSerializer.FromUnsignedInt(data.Buffers[1].Span[0]),
            UInt16Array => ScalarValueSerializer.FromUnsignedInt(MemoryMarshal.Read<ushort>(data.Buffers[1].Span)),
            UInt32Array => ScalarValueSerializer.FromUnsignedInt(MemoryMarshal.Read<uint>(data.Buffers[1].Span)),
            UInt64Array => ScalarValueSerializer.FromUnsignedInt(MemoryMarshal.Read<ulong>(data.Buffers[1].Span)),
            FloatArray => ScalarValueSerializer.FromFloat32(MemoryMarshal.Read<float>(data.Buffers[1].Span)),
            DoubleArray => ScalarValueSerializer.FromFloat64(MemoryMarshal.Read<double>(data.Buffers[1].Span)),
